...
// SSRF validation lives in src/lib/url-safety.ts (shared single source of truth).
...
/** Fetch with timeout, return HTML/text. Validates the URL against SSRF rules first. */
async function fetchText(url: string, timeoutMs = 12000): Promise<string | null> {
  // SSRF defense-in-depth: validate the INITIAL url, not just redirect targets.
  // Addresses stored in the DB are re-validated at fetch time so a bad value
  // that bypassed write-time validation (seed/import/direct DB edit) is still blocked.
  if (!isUrlSafeForFetch(url)) return null;
